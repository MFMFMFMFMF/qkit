    ...
    def run(
        ...
        with lockin.Lockin(
            ...
            **CONVERTER_CONFIGURATION,
            ...
            # tune frequencies
            self.resp_arr = np.zeros(len(self.drive_frequ_arr), np.complex128)
            ...
            for ii in range(len(self.drive_frequ_arr)):
                ...
                self.resp_arr[ii] = np.mean(data[ :])
                dt = time.time()-t0
                minutes = (dt*(len(self.drive_frequ_arr)-ii)/(ii+1))//60
                seconds = round((dt*(len(self.drive_frequ_arr)-ii)/(ii+1))%60)
                print(f"\r freq {round(self.drive_frequ_arr[ii]/1e9,3)} GHz , iteration {ii+1} over {len(self.drive_frequ_arr)} / should end in {minutes} min and {seconds} seconds", end="")
